...
Before(async () => {
  await startWebDriver({ env: process.env.NIGHTWATCH_ENV || 'samsungs9' });
  await createSession();
});
...
AfterAll(async () => {
  await setTimeout(async () => {
    reporter.generate({
      theme: 'foundation',
      jsonFile: 'report/cucumber_report_samsungs9.json',
      output: 'report/cucumber_report_samsungs9.html',
