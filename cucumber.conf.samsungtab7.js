...
Before(async () => {
  await startWebDriver({ env: process.env.NIGHTWATCH_ENV || 'samsungtab7' });
  await createSession();
});
...
AfterAll(async () => {
  await setTimeout(async () => {
    reporter.generate({
      theme: 'foundation',
      jsonFile: 'report/cucumber_report_samsungtab7.json',
      output: 'report/cucumber_report_samsungtab7.html',
