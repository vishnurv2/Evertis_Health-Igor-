...
Before(async () => {
  await startWebDriver({ env: process.env.NIGHTWATCH_ENV || 'samsungtab4' });
  await createSession();
});
...
AfterAll(async () => {
  await setTimeout(async () => {
    reporter.generate({
      theme: 'foundation',
      jsonFile: 'report/cucumber_report_samsungtab4.json',
      output: 'report/cucumber_report_samsungtab4.html',
