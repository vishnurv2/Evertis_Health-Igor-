...
Before(async () => {
  await startWebDriver({ env: process.env.NIGHTWATCH_ENV || 'samsungs20' });
  await createSession();
});
...
AfterAll(async () => {
  await setTimeout(async () => {
    reporter.generate({
      theme: 'foundation',
      jsonFile: 'report/cucumber_report_samsung20.json',
      output: 'report/cucumber_report_samsung20.html',
